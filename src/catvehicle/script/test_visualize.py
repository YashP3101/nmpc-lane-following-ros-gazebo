...
class LaneFollower:
    def __init__(self, init_node=True):
        ...
        # self.navigation_precision_pub = rospy.Publisher('/navigation_precision', Float64, queue_size=10)
        ...
        self.boundary_ema_alpha_mpc = 0.40
        self.boundary_ema_alpha_ppc = 0.40
        ...
        self.csv_file = open('lane_detection_data_mpc.csv', mode='w')
        self.csv_writer = csv.writer(self.csv_file)
        # fo PPC
        # self.csv_writer.writerow(['Timestamp', 'Timestamps(ros_time)', 'Lane_Detection_Accuracy', 'Navigation_Precision', 'Processing_Time', 'steering_angle'])
        # for NMPC
        self.csv_writer.writerow([
            'Timestamp','Steering_Rad','Vref', 'Psi0', 'Psi_Ref','MPC_Solve_ms', 'Pipeline_ms'
        ])
    ...
    def process_point_list(self):
        ...
        _t_pcl_start = time.perf_counter()   # Point cloud processing start
        self.total_frames += 1
        ...
        if points.size == 0:
            ...
            self.lane_detected = False
            # self.log_metrics(False, 0, self.last_steering_angle)
            return
        ...
        rospy.loginfo(f"DBSCAN parameters - eps: {eps_value}, min_samples: {min_samples_value}")
        ...
        _t_pcl_ms = (time.perf_counter() - _t_pcl_start) * 1000
        rospy.loginfo(f"[PCL_TIME] Point Cloud Processing: {_t_pcl_ms:.2f} ms")
    ...
    #     self.log_metrics(self.lane_detected, processing_time, self.last_steering_angle)
        

    # def log_metrics(self, lane_detected, processing_time, steering_angle):
    #     lane_detection_accuracy = (self.successful_detections / self.total_frames) * 100
    #     navigation_precision = self.calculate_navigation_precision()
    #     timestamp = time.time()
    #     timestamps = rospy.Time.now().to_sec()

    #     self.csv_writer.writerow([timestamp, timestamps, lane_detection_accuracy, navigation_precision, processing_time, steering_angle])
    #     self.csv_file.flush()
    ...
    #     deviation = abs(np.arctan(target_slope))
    #     self.navigation_precision_pub.publish(deviation)  # Publish navigation precision
    #     return deviation
    # "============================================"
    ...
    def move_vehicle_mpc(self):
        ...
        v_max = float(self.constant_speed)  # 10
        v_min = 7.5                  
        ...
        self.csv_writer.writerow([
            ...
            psi0,
            ref_traj[0,2],
            (time.perf_counter() - t0) * 1000,        # MPC solve ms
            _t_pipeline_ms 
        ])
        self.csv_file.flush()
    ...
    def run(self):
        ...
        try:
            ...
            if self.use_mpc:
                ...
                # Full plot (boundaries + ref traj + vehicle path)
                fig2, ax2 = plt.subplots()

                # Lane boundaries 
                first_boundary_label = True
                for bl_set in self.boundary_history[::10]:  # every 10th frame
                    for line in bl_set:
                        slope = abs((line[-1,1] - line[0,1]) / (line[-1,0] - line[0,0] + 1e-6))
                        line_plot = line[:len(line)//2] if slope > 0.3 else line  # on turns half the length line
                        ax2.plot(
                            line_plot[:, 0], line_plot[:, 1],
                            'k-', alpha=0.6, linewidth=0.5,
                            label='Lane boundaries' if first_boundary_label else ""
                        )
                        first_boundary_label = False


                # Reference trajectories 
                first_ref_label = True
                for ref in self.ref_traj_history[::20]:  # every 20th frame
                    ax2.plot(
                        ref[:, 0], ref[:, 1],
                        'r-', alpha=0.7, linewidth=0.7,
                        label='Reference trajectory' if first_ref_label else ""
                    )
                    first_ref_label = False

                # Vehicle path
                path_x, path_y = zip(*self.vehicle_path)
                ax2.plot(path_x, path_y, 'b-', linewidth=1, label='Vehicle path')

                # Labels + legend
                ax2.set_xlabel('X [m]')
                ax2.set_ylabel('Y [m]')
                ax2.set_title('MPC Tracking with Lane Boundaries')
                ax2.legend()
                ax2.axis('equal')
                # ax2.grid(True)

                # Save interactive full figure
                with open('/home/student/mpc_plot.pkl', 'wb') as f:
                    pickle.dump(fig2, f)
                rospy.loginfo("pkl saved!")
